...
Mirrors the strategy used in ``test_transform_breweries`` and
``test_ingest_breweries``:
...
from src.silver.transform_brasileirao import (
    ...
    _execute_merge,
    _get_silver_watermark,
    _quarantine_invalid_records,
    _read_bronze_incremental,
    _run_transform,
    transform,
)
...
class TestGetSilverWatermark:
    def test_returns_none_when_table_missing(self):
        ...
        assert _get_silver_watermark(spark) is None
    ...
    def test_returns_hw_value_when_table_populated(self):
        spark = MagicMock()
        spark.catalog.tableExists.return_value = True
        spark.sql.return_value.first.return_value = MagicMock(hw="2026-05-25 12:00:00")
        assert _get_silver_watermark(spark) == "2026-05-25 12:00:00"
...
class TestReadBronzeIncremental:
    def test_returns_none_when_bronze_missing(self):
        spark = MagicMock()
        spark.catalog.tableExists.return_value = False
        assert _read_bronze_incremental(spark, full_refresh=False) is None

    def test_full_refresh_skips_watermark(self):
        spark = MagicMock()
        spark.catalog.tableExists.side_effect = lambda t: t == BRONZE_TABLE
        bronze_df = MagicMock()
        spark.table.return_value = bronze_df

        result = _read_bronze_incremental(spark, full_refresh=True)

        assert result is bronze_df
        # No watermark SQL when full_refresh is True.
        spark.sql.assert_not_called()

    @patch("src.silver.transform_brasileirao._get_silver_watermark", return_value=None)
    def test_no_watermark_returns_full_bronze(self, _hw):
        spark = MagicMock()
        spark.catalog.tableExists.side_effect = lambda t: t == BRONZE_TABLE
        bronze_df = MagicMock()
        spark.table.return_value = bronze_df

        result = _read_bronze_incremental(spark, full_refresh=False)
        # When HW is None, we return the unfiltered Bronze (no filter call).
        assert result is bronze_df
        bronze_df.filter.assert_not_called()

    @patch("src.silver.transform_brasileirao._get_silver_watermark", return_value="2026-05-25 12:00:00")
    def test_with_watermark_filters_bronze(self, _hw, patched_f):
        spark = MagicMock()
        spark.catalog.tableExists.side_effect = lambda t: t == BRONZE_TABLE
        bronze_df = MagicMock()
...
class TestRunTransform:
    @patch("src.silver.transform_brasileirao._read_bronze_incremental", return_value=None)
    def test_short_circuits_when_bronze_missing(self, _read):
        spark = MagicMock()
        # If _read_bronze returns None, _run_transform exits before any
        # MERGE / quarantine call.
        _run_transform(spark, "2026-05-25")
        spark.sql.assert_not_called()

    @patch("src.silver.transform_brasileirao._read_bronze_incremental")
    def test_short_circuits_when_source_empty(self, mock_read):
        empty_df = MagicMock()
...
class TestTransformEntrypoint:
    ...
    def test_full_refresh_passed_through(self, mock_create, mock_run):
        ...
        transform("2026-05-25", full_refresh=True)
